...
function normalizeCollection(collection) {
  ...
  return collection.reduce((all, item, index) => {
    if (!item) return all;
...
function normalizeDb(db) {
  return {
    ...db,
    users: normalizeCollection(db.users),
    quoteRequests: normalizeCollection(db.quoteRequests),
    trainingRecords: normalizeCollection(db.trainingRecords),
    sessions: normalizeCollection(db.sessions)
  };
}
...
function mapCollection(collection) {
  return Object.values(normalizeCollection(collection));
}
...
async function readDb() {
  try {
    ...
    return normalizeDb(readLocalDb());
  }
}
